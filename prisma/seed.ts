...
import { PaymentTerms, TaxMode, TaxScope } from '@prisma/client';

const prisma = new PrismaClient();

function calculateTotals(items: Array<{ quantity: number; unitPrice: number; taxPercent?: number; discountPercent?: number }>, options: { taxMode?: TaxMode; taxScope?: TaxScope; invoiceTaxPercent?: number; shippingAmount?: number } = {}) {
  let subtotal = 0;
  let discountTotal = 0;
  let taxableBase = 0;
  let lineTaxTotal = 0;
...
async function main() {
  ...
  const invoice1Totals = calculateTotals(invoice1Items, { taxMode: TaxMode.SALES_TAX, taxScope: TaxScope.LINE_ITEM, invoiceTaxPercent: 0, shippingAmount: 75 });
  ...
  const invoice2Totals = calculateTotals(invoice2Items, { taxMode: TaxMode.NONE, taxScope: TaxScope.LINE_ITEM, shippingAmount: 0 });
